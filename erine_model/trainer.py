...
class Trainer():
    def __init__(self, model, config, device="cpu"):
        ...
        if config.model_name in ['bert']:
            optimizer = optim.Adam(
                [
                    {"params": model.module.bert.parameters(), "lr": config.lr_bert},
                    {"params": model.module.hidden2tag.parameters(), "lr": config.lr}
                ])
        elif config.model_name in ['erine']:
            optimizer = optim.Adam(
                [
                    {"params": model.module.bert.parameters(), "lr": config.lr_bert},
                    {"params": model.module.project.parameters(), "lr": config.lr},
                    {"params": model.module.emb.parameters(), "lr": config.lr},
                    {"params": model.module.hidden2tag.parameters(), "lr": config.lr},
                    {"params": model.module.linear_layer.parameters(), "lr": config.lr}
                ])
        elif config.model_name in ['erine_agg']:
            optimizer = optim.Adam(
                [
                    {"params": model.module.bert.parameters(), "lr": config.lr_bert},
                    {"params": model.module.project.parameters(), "lr": config.lr},
                    ...
                    {"params": model.module.hidden2tag.parameters(), "lr": config.lr},
                    {"params": model.module.linear_layer.parameters(), "lr": config.lr},
                    {"params": model.module.attentions.parameters(), "lr": config.lr},
                    {"params": model.module.feed_forwards.parameters(), "lr": config.lr},
                    {"params": model.module.layer_norms_1.parameters(), "lr": config.lr},
                    {"params": model.module.layer_norms_2.parameters(), "lr": config.lr}
                ])
        else:
    ...
    def train_epoch(self, train):
        ...
        for idx, batch in enumerate(progress_bar):
            ...
            token_ids, word_input_ids, lenghts, labels = batch

            loss = self.model(token_ids, word_input_ids, lenghts, labels)
    ...
    def test(self, data):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(progress_bar):
                batch = tuple(t.to(self.device) for t in batch)
                token_ids, word_input_ids, lenghts, labels = batch
                label_hat = self.model(token_ids, word_input_ids, lenghts)
